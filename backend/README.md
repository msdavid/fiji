...
3.  **Create an Invitation (using `backend/utils/create-invitation.py`):**
    ...
    *   The script will output an invitation token. This token can be used with the frontend registration page (e.g., `http://localhost:3000/register?token=THE_GENERATED_TOKEN` - adjust port if not default 3000).
...
1.  **Run the Backend (FastAPI with Uvicorn):**
    ...
    *   Start the server (the example uses port 6001; adjust if needed, and ensure `NEXT_PUBLIC_BACKEND_URL` in `frontend/.env.local` matches):
        ```bash
        # In backend/ directory
...
2.  **Run the Frontend (Next.js):**
    ...
    *   Start the Next.js development server. By default, it runs on port 3000:
        ```bash
        # In frontend/ directory
        npm run dev
        ```
    *   To specify a different port (e.g., 3001), use the `-p` flag (note the ` -- ` before the flag, which ensures `npm` passes the flag to the `dev` script):
        ```bash
        # In frontend/ directory
        npm run dev -- -p 3001
        ```
    *   The frontend application should be accessible at `http://localhost:3000` (or the custom port you specified). Check the terminal output for the exact URL.
