...
interface UserAdminCreatePayload {
  email: string;
  // Password is now removed from payload sent by frontend; backend will generate it.
  firstName?: string;
  lastName?: string;
...
interface UserAdminCreateResponse extends UserAdminCreatePayload {
    id: string; // UID of the created user
    generatedPassword?: string; // Backend should return the auto-generated password
}
...
export default function AdminCreateNewUserPage() {
  ...
  const [formData, setFormData] = useState<Omit<UserAdminCreatePayload, 'password'>>({ // Password removed from form state
    email: '',
    firstName: '',
    lastName: '',
  ...
  const fetchAllRoles = useCallback(async () => {
    if (!adminAuthUser || !idToken || !effectiveCanCreate) {
        return;
    }
    try {
        ...
        setPageError(prev => prev ? `${prev}\nFailed to load roles.` : 'Failed to load roles for assignment.');
    }
  }, [adminAuthUser, idToken, effectiveCanCreate]);
  ...
  useEffect(() => {
    ...
    if (adminAuthUser && adminUserProfile) {
        if (!effectiveCanCreate) {
            setPageError("You don't have permission to create new users directly.");
        } else {
            fetchAllRoles();
        }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const responseData = await apiClient<UserAdminCreateResponse>({ // Expect UserAdminCreateResponse
      ...
      if (responseData.generatedPassword) {
        setGeneratedPassword(responseData.generatedPassword); // Store password to display
        successMsg += ` The initial password is: ${responseData.generatedPassword}`;
      } else {
      ...
      setFormData({
        ...
        status: 'pending_verification', assignedRoleIds: [],
      });
    } catch (err: any) {
      setSubmitError(err.response?.data?.detail || err.message || 'Failed to create user account.');
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <section className="border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
            ...
            <div className="space-y-4">
                <div>
                    ...
                    <input type="email" name="email" id="email" value={formData.email} onChange={handleChange} required className="mt-1 block w-full p-3 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-white" />
                </div>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                    An initial password will be auto-generated. The new user should be prompted to change it upon their first login.
                    The generated password will be displayed to you after successful account creation.
                </p>
            </div>
          </section>
          ...
          <section className="border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <label htmlFor="firstName" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">First Name <span className="text-red-500">*</span></label>
                    <input type="text" name="firstName" id="firstName" value={formData.firstName || ''} onChange={handleChange} required className="mt-1 block w-full p-3 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-white" />
                </div>
                <div>
                    <label htmlFor="lastName" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Last Name <span className="text-red-500">*</span></label>
                    <input type="text" name="lastName" id="lastName" value={formData.lastName || ''} onChange={handleChange} required className="mt-1 block w-full p-3 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-white" />
                </div>
                <div>
                    <label htmlFor="status" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Account Status</label>
                    <select name="status" id="status" value={formData.status} onChange={handleChange} className="mt-1 block w-full p-3 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-white">
          ...
          {allRoles.length > 0 && (
            <section>
                <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Assign Roles (Optional)</h2>
          ...
          {submitSuccess && (
            <div className="my-4 p-3 text-sm text-green-600 bg-green-50 dark:bg-green-900/30 rounded-lg" role="alert">
              <div className="flex items-center mb-2">
                <span className="material-icons text-lg mr-2">check_circle_outline</span>
                User account created successfully for {formData.email || 'the user'}.
              </div>
              {generatedPassword && (
                <div className="mt-2 p-2 bg-gray-100 dark:bg-gray-800 rounded">
                  <p className="text-xs text-gray-700 dark:text-gray-300">Initial auto-generated password:</p>
                  <p className="font-mono text-sm text-indigo-700 dark:text-indigo-400 break-all">{generatedPassword}</p>
                  <p className="mt-1 text-xs text-yellow-700 dark:text-yellow-500">Please provide this password to the user and advise them to change it immediately upon first login.</p>
                </div>
              )}
            </div>
          )}
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700 mt-6">
            ...
            <button
              type="submit"
              disabled={isSubmitting || !effectiveCanCreate}
            ...
            >
              {isSubmitting ? (
                <><span className="material-icons animate-spin text-base mr-2">sync</span>Creating User...</>
              ) : (
                <><span className="material-icons text-base mr-2">person_add</span>Create User Account</>
              )}
            </button>
          </div>
        </form>
