...
// Re-use or import ALL_PRIVILEGES definition
const ALL_PRIVILEGES: Record<string, { name: string; actions: { id: string; label: string }[] }> = {
  users: {
    name: 'Users',
    actions: [
      { id: 'list', label: 'List Users' }, { id: 'view', label: 'View User Details' },
      { id: 'edit', label: 'Edit Users' }, { id: 'delete', label: 'Delete Users' },
    ],
  },
  roles: {
    name: 'Roles',
    actions: [
      { id: 'list', label: 'List Roles' }, { id: 'create', label: 'Create Roles' },
      { id: 'view', label: 'View Role Details' }, { id: 'edit', label: 'Edit Roles' },
  ...
  events: {
    name: 'Events',
    actions: [
      { id: 'list', label: 'List Events' }, { id: 'create', label: 'Create Events' },
      { id: 'view', label: 'View Event Details' }, { id: 'edit', label: 'Edit Events' },
      { id: 'delete', label: 'Delete Events' }, { id: 'manage_assignments', label: 'Manage Event Assignments' },
    ],
  },
  working_groups: {
    name: 'Working Groups',
    actions: [
      { id: 'list', label: 'List Working Groups' }, { id: 'create', label: 'Create Working Groups' },
      { id: 'view', label: 'View WG Details' }, { id: 'edit', label: 'Edit Working Groups' },
      { id: 'delete', label: 'Delete Working Groups' }, { id: 'manage_assignments', label: 'Manage WG Members' },
    ],
  },
  donations: {
    name: 'Donations',
    actions: [
      { id: 'list', label: 'List Donations' }, { id: 'create', label: 'Create Donations' },
      { id: 'view', label: 'View Donation Details' }, { id: 'edit', label: 'Edit Donations' },
  ...
  invitations: {
    name: 'Invitations',
    actions: [
      { id: 'list', label: 'List Invitations' }, { id: 'create', label: 'Create Invitations' },
      { id: 'delete', label: 'Delete Invitations' },
    ],
  },
  admin: {
    name: 'Admin Functions',
    actions: [{ id: 'view_summary', label: 'View Admin Dashboard Summary' }],
  },
};
...
export default function EditRolePage() {
  ...
  const fetchRoleDetails = useCallback(async () => {
    if (!user || !idToken || !roleId || roleId === 'undefined') {
      if (roleId === 'undefined') {
        setPageError("Invalid role ID provided. Cannot fetch details.");
      }
      setIsLoading(false);
      return;
    }
    
    if (userProfile && !canEditRoles) {
    ...
    try {
      const fetchedRole = await apiClient<RoleData>({
        path: `/roles/${roleId}`,
        token: idToken,
        method: 'GET',
      });
      setOriginalRole(fetchedRole);
      setRoleName(fetchedRole.roleName);
      setDescription(fetchedRole.description || '');
      setSelectedPrivileges(fetchedRole.privileges || {});
    } catch (err: any) {
      setPageError(err.response?.data?.detail || err.message || 'Failed to fetch role details.');
      console.error("Fetch role error:", err);
    } finally {
      setIsLoading(false);
    }
  }, [user, idToken, roleId, userProfile, canEditRoles]); 
  ...
  const handlePrivilegeChange = (resource: string, action: string, checked: boolean) => {
    setSelectedPrivileges(prev => {
      ...
      if (checked) {
        if (!updatedResourcePrivileges.includes(action)) updatedResourcePrivileges.push(action);
      } else {
        const index = updatedResourcePrivileges.indexOf(action);
        if (index > -1) updatedResourcePrivileges.splice(index, 1);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (originalRole.roleName.toLowerCase() === 'sysadmin' && roleName.toLowerCase() !== 'sysadmin') {
        setSubmitError("The 'sysadmin' role name cannot be changed.");
        return;
    }
    ...
    if (JSON.stringify(selectedPrivileges) !== JSON.stringify(originalRole.privileges)) {
        payload.privileges = selectedPrivileges;
    }
    ...
    } catch (err: any) { // Corrected line: added curly braces for the catch block
      setSubmitError(err.response?.data?.detail || err.message || 'Failed to update role.');
  ...
  const handleDeleteRole = async () => {
    if (!originalRole || !canDeleteRoles || !idToken) {
        setDeleteError("Cannot delete: Insufficient permissions or role data missing.");
        return;
    }
    if (originalRole.roleName.toLowerCase() === 'sysadmin') {
        setDeleteError("The 'sysadmin' role cannot be deleted.");
        return;
    }
    ...
    try {
        ...
        setDeleteError(err.response?.data?.detail || err.message || 'Failed to delete role.');
      ...
      <div className="bg-white dark:bg-gray-900 shadow-xl rounded-xl p-6 sm:p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-6">
            ...
            {isSysAdminRole && (
                <div className="mb-4 p-3 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-700 rounded-md text-sm text-indigo-700 dark:text-indigo-300">
                    <span className="material-icons text-lg mr-2 align-middle">info_outline</span>
                    The 'sysadmin' role inherently has all permissions. Modifications are not recommended and may be restricted.
                </div>
            )}
        ...
        {canDeleteRoles && originalRole && !isSysAdminRole && (
            <div className="mt-10 pt-6 border-t border-red-300 dark:border-red-700">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                    Deleting this role is permanent. Users assigned this role will lose its associated permissions.
                </p>
                <button
                ...
                >
                    ...
                    {isDeleting ? 'Deleting Role...' : 'Delete This Role'}
                </button>
                {deleteError && 
