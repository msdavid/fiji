...
// Interfaces (consider moving to a shared models directory)
...
interface UserAssignment { 
  ...
  userEmail?: string;
  assignedAt?: string; 
  // Potentially, the API might return top-level fields like 'id' (for user's ID), 'firstName', 'email'
  // if it's returning a list of user objects directly.
  id?: string; // If API returns user's ID as 'id'
  firstName?: string; // If API returns first name as 'firstName'
  lastName?: string; // If API returns last name as 'lastName'
  email?: string; // If API returns email as 'email'
}
...
export default function RoleDetailPage() {
  ...
  const fetchRoleDetailsCallback = useCallback(async () => {
    ...
    try {
      const fetchedRole = await apiClient<RoleDetails>({
        path: `/roles/${roleId}`, 
        token: idToken,
        method: 'GET',
      });
      setRoleDetails(fetchedRole);
    } catch (err: any) { 
  ...
  }, [user, idToken, roleId, userProfile, canViewRole]);

  const fetchAssignedUsers = useCallback(async () => {
    if (!user || !idToken || !roleId || !canManageRoleAssignments || roleId === 'undefined') {
        return;
    }
    setActionInProgress(true); setAssignmentError(null); 
    console.log(`Fetching assigned users for roleId: ${roleId}`);
    try {
      const usersWithRole = await apiClient<UserAssignment[]>({ 
        path: `/users?roleId=${roleId}`, 
        token: idToken,
        method: 'GET',
      });
      console.log("Raw data from /users?roleId API:", usersWithRole); // Log raw data
      
      // Map data if necessary, e.g., if API returns 'id' but we need 'userId' for consistency in UserAssignment
      const processedUsers = usersWithRole.map(u => ({
        userId: u.id || u.userId, // Prefer 'id' if it's the user's main ID, fallback to userId
      ...
      console.log("Processed assigned users for state:", processedUsers);
      setAssignedUsers(processedUsers);

    } catch (err: any) { 
      setAssignmentError(err.response?.data?.detail || err.message || 'Failed to fetch assigned users.');
      console.error("Fetch assigned users error:", err);
    } 
    finally { setActionInProgress(false); }
  }, [user, idToken, roleId, canManageRoleAssignments]);
  ...
  useEffect(() => {
    ...
    if (user && userProfile && idToken && roleId && roleId !== 'undefined') { 
      ...
      if (canManageRoleAssignments) fetchAssignedUsers();
        ...
        setPageError("Invalid role ID provided.");
        setIsLoading(false);
    }
  ...
  const handleAssignUserToRole = async (e: React.FormEvent) => {
    ...
    if (!user || !idToken || !roleId || !selectedUserToAssign || !canManageRoleAssignments) {
        setAssignmentError("Please select a user to assign.");
        return;
    }
    if (assignedUsers.some(au => au.userId === selectedUserToAssign.id)) {
        setAssignmentError(`${selectedUserToAssign.firstName || selectedUserToAssign.email} is already assigned to this role.`);
        return;
    }
    ...
    try {
      await apiClient({
        path: `/users/${selectedUserToAssign.id}/roles/${roleId}`, 
        token: idToken,
        method: 'POST', 
      });
      ...
      setAssignmentError(err.response?.data?.detail || err.message || 'Failed to assign user to role.');
      console.error("Assign user to role error:", err);
    }
    finally { setActionInProgress(false); }
  };
  ...
  const handleUnassignUserFromRole = async (userIdToUnassign: string) => {
    ...
    try {
      await apiClient({
        path: `/users/${userIdToUnassign}/roles/${roleId}`, 
        token: idToken,
        method: 'DELETE',
      });
      await fetchAssignedUsers(); 
    } catch (err: any) { 
      setAssignmentError(err.response?.data?.detail || err.message || 'Failed to unassign user from role.');
      console.error("Unassign user from role error:", err);
    }
    finally { setActionInProgress(false); }
  };
        ...
        {roleDetails && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <DetailItem
                    icon="description"
                    label="Description"
                    value={roleDetails.description || <span className="italic text-gray-400 dark:text-gray-500">No description</span>}
                />
                <DetailItem
                    icon="calendar_today"
                    label="Created On"
                    value={format(parseISO(roleDetails.createdAt), 'PPpp')}
                />
                <DetailItem
                    icon="update"
                    label="Last Updated"
                    value={format(parseISO(roleDetails.updatedAt), 'PPpp')}
                />
            </div>
            ...
            {canManageRoleAssignments && (
                ...
                {actionInProgress && assignedUsers.length === 0 && !assignmentError ? (
                    ...
                    {assignedUsers.map((assignedUser) => (
                        ...
                        <div>
                            ...
                            {assignedUser.userFirstName || 'N/A'} {assignedUser.userLastName || ''} 
                            <span className="text-sm text-gray-600 dark:text-gray-400 ml-1">({assignedUser.userEmail || assignedUser.userId})</span>
                            </p>
                            {assignedUser.assignedAt && (
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                    Assigned: {format(parseISO(assignedUser.assignedAt), 'PP p')}
                                </p>
                            )}
                        </div>
                        <button
                            onClick={() => handleUnassignUserFromRole(assignedUser.userId)}
                ...
                {assignmentError && assignedUsers.length === 0 && !actionInProgress &&
                    <div className="mt-3 p-3 text-sm text-red-600 bg-red-50 dark:bg-red-900/30 rounded-md flex items-center" role="alert">
                    <span className="material-icons text-lg mr-2">error_outline</span>
                    {assignmentError}
                    </div>
